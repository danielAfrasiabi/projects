...
public class BusStopTree {
  ...
  public static boolean isValidBST(Node<Bus> node) {
    ...
    Bus valueOfNode = node.getValue();

    // Check validity of left and right subtrees
    if (!isValidBSTHelper(node.getLeft(), null, valueOfNode)
        || !isValidBSTHelper(node.getRight(), valueOfNode, null)) {
      // If either left or right subtree is invalid, then entire BST is invalid
      return false;
    }

    // Use recursion to test entire BST
   ...
   * 
   * @return true if BST is valid, false if not
   */
  private static boolean isValidBSTHelper(Node<Bus> node, Bus min, Bus max) {
    // Empty tree is a valid BST
    if (node == null) {
      return true;
    }
    ...
    // Check if current node's value is within allotted range
    if ((min != null && valueOfNode.compareTo(min) <= 0)
        || (max != null && valueOfNode.compareTo(max) >= 0)) {
      return false;
    }

    // Use recursion to test entire BST
    return isValidBSTHelper(node.getLeft(), min, valueOfNode)
        && isValidBSTHelper(node.getRight(), valueOfNode, max);
  }
  ...
  public boolean addBus(Bus bus) {
    // Check if the bus goes to the specified stop
    if (!bus.goesTo(userStop)) {
      throw new IllegalArgumentException("Attempt to add a bus that does not go to the stop.");
    }

    // If the tree is empty, create a new root node with the given bus
    if (root == null) {
      root = new Node<Bus>(bus);
      return true;
    }

    // Call the helper method to recursively add the bus to the tree
  ...
  public boolean addBusHelper(Node<Bus> node, Bus bus) {
    ...
    if (node == null) {
      node = new Node<Bus>(bus);
      return true;
    }

    // If the bus is greater than the current node's bus, go to the right subtree
    if (bus.compareTo(node.getValue()) > 0) {
      // If the right child is null, create a new node with the given bus
    ...
    // If the bus is less than the current node's bus, go to the left subtree
    else if (bus.compareTo(node.getValue()) < 0) {
      // If the left child is null, create a new node with the given bus
    ...
    // If the bus is equal to the current node's bus, do not allow duplicate buses
    else {
      return false;
    }
  }
  ...
  public boolean contains(Bus bus) {
    // An empty BST contains zero elements
    if (root == null) {
      return false;
    }
  ...
  public boolean containsHelper(Node<Bus> node, Bus bus) {
    // Positive if bus is greater than node's bus
    int comparisonValue = bus.compareTo(node.getValue());
    ...
    // Bus is greater than node's bus, move to the right
    else if (comparisonValue > 0) {
      // BST does not contain bus
      if (node.getRight() == null) {
        return false;
      }
    ...
    // Bus is less than node's bus, move to the left
    else {
      // BST does not contain bus
      if (node.getLeft() == null) {
        return false;
  ...
  protected Node<Bus> getFirstNodeAfter(LocalTime time, Node<Bus> node) {
    ...
    // Compare the arrival time of the current node with the specified time
    int comparisonValue = node.getValue().getArrivalTime().compareTo(time);

    // If the current node's arrival time is before the specified time, go to the right subtree
    if (comparisonValue < 0) {
    ...
    // If the current node's arrival time is after the specified time, go to the left subtree
    else if (comparisonValue > 0) {
      // If the left child is null, return the current node
      if (node.getLeft() == null) {
        return node;
      }
      // If the left child's arrival time is before the specified time, return the current node
    ...
    // If the current node's arrival time is equal to the specified time, return the current node
    else {
      return node;
    }
  }
  ...
  public Bus getFirstBus() {
    ...
    // Call the helper method to get the first node with the smallest value
    Node<Bus> node = getFirstBusHelper(root);

    // If no node is found, return null; otherwise, return the value of the node
    if (node == null) {
      return null;
    }
    return node.getValue();
  ...
  public static Node<Bus> getFirstBusHelper(Node<Bus> node) {
    // If there is a left child, recursively call the method on the left subtree
    if (node.getLeft() != null) {
      return getFirstBusHelper(node.getLeft());
    }
    // If there is no left child, return the current node (the leftmost node)
    return node;
  }
  ...
  public Bus getLastBus() {
    ...
    // Call the helper method to get the last node with the largest value
  ...
  public static Node<Bus> getLastBusHelper(Node<Bus> node) {
    // If there is a right child, recursively call the method on the right subtree
    if (node.getRight() != null) {
      return getLastBusHelper(node.getRight());
    }
    // If there is no right child, return the current node (the rightmost node)
    return node;
  }
  ...
  private int heightHelper(Node<Bus> node) {
    ...
    // Base case: if the current node is a leaf (no left or right child), the height is 1
    if (node.getRight() == null && node.getLeft() == null) {
      return 1;
    }

    // Recursively calculate the height of the subtree, taking the maximum height of left and right
    // subtrees
    return 1 + Math.max(heightHelper(node.getRight()), heightHelper(node.getLeft()));
  }
}
