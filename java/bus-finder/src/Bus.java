...
public class Bus implements Comparable<Bus> {
  ...
  public int compareTo(Bus other) {
    // positive if greater, negative if less than
    int result1 = arrivalTime.compareTo(other.arrivalTime);

    // positive if greater, negative if less than
    int result2 = route.getRouteAndTripName().compareTo(other.route.getRouteAndTripName());

    // indicates arrival time of this bus is later than arrival time of other bus
    if (result1 > 0) {
      return 1;
    }

    // indicates arrival time of this bus is earlier than arrival time of other bus
    if (result1 < 0) {
      return -1;
    }

    // indicates route and trip name of this bus is alphabetically greater than route and trip name
    // of other bus
    if (result2 > 0) {
      return 1;
    }

    // indicates route and trip name of this bus is alphabetically less than route and trip name
    // of other bus
    if (result2 < 0) {
      return -1;
    }
    ...
    // INCOMING > OUTGOING
    if (route.getDirection().equals(BusRoute.BusDirection.INCOMING)
        && other.route.getDirection().equals(BusRoute.BusDirection.OUTGOING)) {
      return 1;
    }

    // OUTGOING < OUTGOING
    if (route.getDirection().equals(BusRoute.BusDirection.OUTGOING)
        && other.route.getDirection().equals(BusRoute.BusDirection.INCOMING)) {
      return -1;
    }
